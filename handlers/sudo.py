...
def sudo(client, message,redis):
	...
	if text and (type is enums.ChatType.PRIVATE or (type is enums.ChatType.SUPERGROUP or type is enums.ChatType.GROUP)) :
		...
		if rank is "sudo" or rank is "asudo":
			...
			if text == c.ADDfiles:
				url = "https://raw.githubusercontent.com/Marawan092/debo-files/master/files"
			...
			if text == c.Ubot:
				...
				for fnh in Files_H:
					url = "https://raw.githubusercontent.com/Marawan092/debo/master/handlers/"+fnh
					out = requests.get(url).text
					f = open("./handlers/"+fnh,"w+")
					f.write(out)
					f.close()
				for fnu in Files_U:
					url = "https://raw.githubusercontent.com/Marawan092/debo/master/utlis/"+fnu
					out = requests.get(url).text
					f = open("./utlis/"+fnu,"w+")
					f.write(out)
					f.close()
				for fnb in Files_B:
					url = "https://raw.githubusercontent.com/Marawan092/debo/master/"+fnb
					out = requests.get(url).text
					f = open("./"+fnb,"w+")
					f.write(out)
					f.close()
				for fnu in Files_L:
					url = "https://raw.githubusercontent.com/Marawan092/debo/master/lang/"+fnu
					out = requests.get(url).text
					f = open("./lang/"+fnu,"w+")
					f.write(out)
					f.close()
